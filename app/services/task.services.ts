...
import PhaseStatus from '../enums/phase.enums';
  ...
  const getPhaseIndex = (phase: IPhase): number => phasesDummy.findIndex((el) => el.phaseId === phase.phaseId);
  ...
  export const updateTaskStatus = (phaseId: number, taskId: number): ITask => {
    const phase: IPhase = fetchSinglePhase(phaseId);
    ...
    const task: ITask = fetchPhaseTask(phase.phaseId, taskId);
    if (task.isComplete) {
